...
class AcronymGrasps:
    ...
    def load_mesh(self):
        ...
        mesh = trimesh.load(mesh_path_file, file_type="obj")
...
class AcronymAndSDFDataset(Dataset):
    ...
    def __init__(
        ...
        for grasp_file in grasps_files:
            ...
            sdf_file = os.path.join(
                self.sdf_dir, class_type, txt_split[-1].split(".")[0] + ".json"
            )
            point_file = os.path.join(
                self.generated_points_dir,
                class_type,
                "4_points",
                txt_split[-1] + ".npz",
            )
    ...
    def _get_item(self, index):
        ...
        ## Grasps good/bad
        rix = np.random.randint(
            low=0, high=grasps_obj.good_grasps.shape[0], size=self.n_density
        )
        H_grasps = grasps_obj.good_grasps[rix, ...]
        rix = np.random.randint(
            low=0, high=grasps_obj.bad_grasps.shape[0], size=self.n_density
        )
        ...
        res = {
            "point_cloud": torch.from_numpy(p_clouds).float(),
        ...
        return res, {
            ...
            "grad_sdf": torch.from_numpy(grad_sdf).float(),
        }
...
class PointcloudAcronymAndSDFDataset(Dataset):
    ...
    def __init__(
        ...
        for class_type_i in class_type:
            cls_grasps_files = sorted(
                glob.glob(self.grasps_dir + "/" + class_type_i + "/*.h5")
            )
    ...
    def _get_grasps(self, grasp_obj):
        try:
            rix = np.random.randint(
                low=0, high=grasp_obj.good_grasps.shape[0], size=self.n_density
            )
        except:
            print("lets see")
...
class PartialPointcloudAcronymAndSDFDataset(Dataset):
    ...
    def __init__(
        self,
        class_type=["Cup", "Mug", "Fork", "Hat", "Bottle"],
        se3=False,
        phase="train",
        one_object=False,
        n_pointcloud=1000,
...
if __name__ == "__main__":
    ...
    dataset = AcronymAndSDFDataset(
        visualize=True, augmented_rotation=True, one_object=False
    )

    ## Pointcloud conditioned dataset
    dataset = PointcloudAcronymAndSDFDataset(
        visualize=True, augmented_rotation=True, one_object=False
    )

    ## Pointcloud conditioned dataset
    dataset = PartialPointcloudAcronymAndSDFDataset(
        visualize=False, augmented_rotation=True, one_object=False
    )
